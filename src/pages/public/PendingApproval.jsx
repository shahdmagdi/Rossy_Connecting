...
const PendingApproval = () => {
  ...
  // If doctor is already approved, redirect immediately
  useEffect(() => {
  ...
  const handleCheckStatus = async () => {
    ...
    try {
      ...
      const vs = updatedUser.verification_status;
  ...
  const isRejected = user?.verification_status === 'rejected' || statusType === 'rejected';
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#FCE7F3', padding: '20px' }}>
      <div style={{ backgroundColor: '#ffffff', padding: '48px', borderRadius: '16px', boxShadow: '0 10px 40px rgba(219,39,119,0.15)', width: '100%', maxWidth: '480px', textAlign: 'center' }}>
        ...
        <div style={{ width: '100px', height: '100px', borderRadius: '50%', backgroundColor: isRejected ? '#FEE2E2' : '#FEF3C7', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px', fontSize: '48px' }}>
          {isRejected ? '❌' : '⏳'}
        </div>

        <h2 style={{ fontSize: '24px', fontWeight: '700', color: isRejected ? '#991B1B' : '#92400E', marginBottom: '16px' }}>
          {isRejected ? 'Account Not Approved' : 'Account Pending Approval'}
        </h2>

        {/* Status box */}
        <div style={{ backgroundColor: isRejected ? '#FEE2E2' : '#FEF3C7', border: `1px solid ${isRejected ? '#EF4444' : '#F59E0B'}`, borderRadius: '12px', padding: '24px', marginBottom: '24px' }}>
          <h3 style={{ fontSize: '18px', fontWeight: '600', color: isRejected ? '#991B1B' : '#92400E', marginBottom: '8px' }}>
            {isRejected ? 'Application Rejected' : 'Under Review'}
          </h3>
          <p style={{ fontSize: '14px', color: isRejected ? '#B91C1C' : '#B45309', lineHeight: '1.6' }}>
            {isRejected
              ? 'Unfortunately your doctor registration was not approved. Please contact support for more information.'
              : 'Our admin team is reviewing your doctor registration. You\'ll be notified by email once your account has been approved.'
            }
          </p>
        </div>
        ...
        <div style={{ fontSize: '14px', color: '#9D174D', marginBottom: '24px' }}>
          <p>Name: <strong style={{ color: '#831843' }}>{user?.full_name || 'Doctor'}</strong></p>
          <p>Email: <strong style={{ color: '#831843' }}>{user?.email || '—'}</strong></p>
          <p>Status: <strong style={{ color: isRejected ? '#991B1B' : '#92400E' }}>
        ...
        {statusMessage && (
          <div style={{ marginTop: '16px', padding: '12px', borderRadius: '8px', backgroundColor: statusBg, color: statusColor, fontSize: '14px', fontWeight: '500' }}>
            {statusMessage}
          </div>
        )}

        {/* What happens next — only show when pending */}
        {!isRejected && (
